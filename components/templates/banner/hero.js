...
export default function Hero({
    ...
    const content = (
        <div className={"flex flex-col md:flex-row relative"} style={{backgroundColor: first ? first : ""}}>
            ...
            <div className={"w-full md:w-1/3 flex items-center order-1 md:order-2 pt-7 md:pt-0"}>
                <div className={"text-center md:max-w-md m-auto"}>
                    ...
                    <HeaderTitle color={titleColor} tag={"h2"} size={"text-3xl md:text-7xl"} position={"center"} style={"utopia"}
                    ...
                    {
                        button?(
                            <div className="flex justify-center pt-12 absolute bottom-10 w-full md:bottom-auto md:w-auto  md:relative">
                                <Button label={button.title} size={"medium"} color={"gray_2"} onClick={buttonOnClick} className={"w-11/12 md:w-auto text-sm tracking-widest font-normal"}/>
                            </div>
                        ):null
                    }
                </div>
