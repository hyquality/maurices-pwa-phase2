...
export default function InstaSlide(
    ...
    const content = (
        <div
            className={`overflow-hidden justify-center ${paddingTop && "pt-sectionBT"} ${paddingBottom && "pb-sectionBT"}`}>
            <div>
                ...
                <Carousel templates={templates} context={"slider-insta-carousel"} visibleNum={5} mobileVisibleNum={2}
                          showScrollMobile={true} showNavMobile={false} showNav={false} loop={false} spaceBetween={10}/>

                <Button className={`mx-auto mt-12`} onClick={onClick} color={'gray_2'} size={`medium`}
                        label={button.title}/>
            </div>
        </div>
    )
